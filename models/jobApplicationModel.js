...
// Generate unique application number (Mongoose 8+: async middleware must not use `next`)
...
// Validate that only students can apply (Mongoose 8+: async middleware must not use `next`)
jobApplicationSchema.pre('save', async function () {
  if (this.isNew) {
...
// Validate that job post belongs to a client (Mongoose 8+: async middleware must not use `next`)
jobApplicationSchema.pre('save', async function () {
  if (this.isNew) {
